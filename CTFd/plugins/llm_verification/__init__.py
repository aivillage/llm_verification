...
def generate_text(prompt):
    ...
    generated_text = json_response[0]['generated_text'][len(prompt):]
...
def load(app):
    ...
    register_plugin_assets_directory(app,
                                     base_path='/plugins/llm_verification/assets/')
    llm_verifications = Blueprint('llm_verifications',
                                  __name__,
                                  template_folder='templates')
    ...
    def generate_for_challenge():
        ...
        try:
            ...
            print(text)
            resp = {'success': True, 'data': {'text': text}}
            return jsonify(resp)
